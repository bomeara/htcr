...
#' job_number <- args[1]
#' set.seed(round(runif(1, 1, 10000) + as.numeric(job_number), 0)) #to make sure we don't have jobs use the same exact seed if they fire at the same time
#' x_mean <- args[2]
#' x_sd <- args[3]
#' load("my_data.rda") # which loads the data. We could instead do read.csv, etc.
#' lnl <- dnorm(data_points, mean=x_mean, sd=x_sd, log=TRUE)
#' save(job_number, x_mean, x_sd, lnl, file=paste0("Results_Job",job_number,"_Mean_",signif(x_mean,2), "_SD_", signif(x_sd, 2), ".rda"))
#'
...
fly <- function(conditions, rscript, reps=1, input_files=NULL, sleep_time=1, log='log.$(Process)', output='out.$(Process)', error='error.$(Process)', universe="vanilla", requirements=NULL, other=NULL, submit_file_name="Rjob.submit") {
  ...
  executable_file_create(rscript=rscript, executable=executable)
  for (condition.index in sequence(nrow(conditions))) {
    submit_file_full <- paste0(submit_file_name, ".", condition.index)
    submit_file_create(input, reps, executable=executable, log=log, output=output, error=error, universe=universe, requirements=requirements, other=other, submit_file_name=submit_file_full)
